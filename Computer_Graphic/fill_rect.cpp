...
void Fill_Rect::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,QWidget *widget){
    ...
    drawRect(painter,start,end);
    painter->setBrush(brush());
    drawfill_rect(painter,start,end);
...
void Fill_Rect::mousePressEvent(QGraphicsSceneMouseEvent *event){
...
void Fill_Rect::hoverMoveEvent(QGraphicsSceneHoverEvent *event){
...
void Fill_Rect::mouseMoveEvent(QGraphicsSceneMouseEvent *event){
    if(If_br_resizing){
        setEnd(event->pos());
        setRect(getRect());
    }
...
void Fill_Rect::mouseReleaseEvent(QGraphicsSceneMouseEvent *event){
    cur_rect = getRect();
    if(If_br_resizing){
        If_br_resizing = false;
...
QRectF Fill_Rect::boundingRect() const{
...
qreal Fill_Rect::rotating(QPointF x, QPointF y, QPointF z){
    qreal cosA;
...
void Fill_Rect::startDraw(QGraphicsSceneMouseEvent* event){
    setRect(QRectF(event->scenePos(),QSizeF(0,0)));
}

void Fill_Rect::drawing(QGraphicsSceneMouseEvent* event){
    setRect(getRect());
}

int Fill_Rect::type() const{
    return UserType + 1;
}
